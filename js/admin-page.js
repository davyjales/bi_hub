...
(() => {
  ...
  async function reloadUsersTable() {
    ...
    if (!users.length) {
      ...
      tr.innerHTML =
        '<td colspan="5" class="px-4 py-6 text-center text-sm text-[var(--text-muted)]">Sem utilizadores</td>';
  ...
  function beginEdit(uid) {
    ...
    toggleEditDirs();
    const ids = Array.isArray(u.directoryIds) ? u.directoryIds.map(Number).filter((x) => x > 0) : [];
